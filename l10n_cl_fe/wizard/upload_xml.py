...
<RecepcionEnvio>
    ...
    def _create_partner(self, data):
        if self.pre_process and self.type == 'compras':
            return False
        type = "Emis"
        correo = 'facturacionmipyme@sii.cl'
        if self.type == 'ventas':
            type = "Recep"
            try:
                correo = data['CorreoReceptor'] 
            except:
                correo = 'facturacionmipyme@sii.cl'
        else:
            try:
                correo = data['CorreoEmisor'] 
            except:
                correo = 'facturacionmipyme@sii.cl'
    ...
    def _prepare_line(self, line, document_id, account_id, type, price_included=False):
        ...
        if isinstance(product_id, int):
            _logger.warning('INSTANCE')
            #Buscamos el modelo producto, ya que lo que tenemos es un entero
            producto = self.env['product.product'].search([('product_tmpl_id', '=', product_id),], limit=1)
            #Tenemos producto, ahora buscamos la cuenta contable. En el caso de que no encuentre trabajamos con account_id
            _logger.warning('DUMMIE')
            account_dummie = producto.property_account_expense_id.id if producto.property_account_expense_id.id else account_id
            if type == 'ventas':
                account_dummie = producto.property_account_income_id.id if producto.property_account_income_id.id else account_id 
            _logger.warning('PASAMOS PRUEBA')
    ...
    def _get_data(self, documento, company_id):
        ...
        categoria = self.env['product.category'].search([('name', '=', 'All')],limit=1)
        if not categoria:
