...
class AccountInvoice(models.Model):
    ...
    def _compute_amount(self):
        for inv in self:
            ...
            if included:
                ...
                neto += sum(line.price_subtotal for line in inv.invoice_line_ids if line.account_id)
    ...
    def action_invoice_cancel(self):
        if self.sii_result not in [False, 'draft', 'NoEnviado', 'Rechazado', 'Anulado']:
            raise UserError(_("No se puede Cancelar un documento aceptado en el SII"))
...
{}
    ...
    def sign_full_xml(self, message, uri, type='doc'):
        #Esto debemos mejorar. Buscamos a la primera que consiga con certificado    
        obj = self.env['res.company'].search([('status', '=', 'valid')], limit=1)
        signature_id = {
...
#                lines['RUTMandante']
    ...
    def get_sii_result(self):
        for r in self:
            try:
                ...
                _logger.warning("NO hay Respuesta SII MESSAGE: %s" %str(e))
    ...
    def _get_dte_status(self):
        for r in self:
            ...
            date_invoice = datetime.strptime(str(r.date_invoice), "%Y-%m-%d").strftime("%d-%m-%Y")
